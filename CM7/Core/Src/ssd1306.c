...
I2C_HandleTypeDef HandleToI2CDisp;
...
uint8_t SSD1306_Init(void) {
	...
	if (HAL_I2C_IsDeviceReady(&HandleToI2CDisp, SSD1306_I2C_ADDR, 1, 20000) != HAL_OK) {
		/* Return false */
		return 0;
	}
...
void ssd1306_I2C_WriteMulti(uint8_t address, uint8_t reg, uint8_t *data, uint16_t count) {
	...
	for (i = 0; i < count; i++)
		dt[i + 1] = data[i];
	HAL_I2C_Master_Transmit(&HandleToI2CDisp, address, dt, count + 1, 10);
}
...
void ssd1306_I2C_Write(uint8_t address, uint8_t reg, uint8_t data) {
	...
	dt[0] = reg;
	dt[1] = data;
	HAL_I2C_Master_Transmit(&HandleToI2CDisp, address, dt, 2, 10);
}

void SSD1306_setI2CSTMHandle(I2C_HandleTypeDef *handleToI2CDisp){
	HandleToI2CDisp = *handleToI2CDisp;
}
